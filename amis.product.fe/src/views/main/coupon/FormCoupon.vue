        ...
        <div class="form-item_input">
          ...
          <div class="form-group ms-small">
            <base-input
              ...
              :type="'text'"
              :maxLength="25"
              :messageValid="t('validate.empty', { field: t('module.order.couponCode') })"
          ...
          <div class="form-group ms-big">
            <base-input
              ...
              :type="'text'"
              :maxLength="255"
              :messageValid="t('validate.empty', { field: t('module.order.couponName') })"
        ...
        <div class="form-item_input">
          <div class="form-group ms-small">
            <base-input
              :required="true"
              :type="'text'"
              :messageValid="t('validate.empty', { field: t('module.order.percent') })"
          ...
          <div class="form-group ms-big">
            <base-input
              ...
              :type="'text'"
              :isNumber="true"
              :messageValid="t('validate.empty', { field: t('module.order.quantity') })"
        ...
        <div class="form-item_input">
          <div class="form-group ms-small">
            <base-calendar
              ...
              :messageValid="t('validate.empty', { field: t('module.order.dateStart') })"
          ...
          <div class="form-group ms-big">
            <base-calendar
              ...
              :messageValid="t('validate.empty', { field: t('module.order.dateEnd') })"
...
import { ActionTable, Coupon, ENotificationType, Grid, ModuleName, ServiceResponse } from '@/core/public_api';
...
async function handleSaveData(closeModal: any) {
  try {
    ...
    if (messValid.length > 0) {
      isValid.value = true;
      unListenEvent();
      props.Base.showNotificationError(messValid, ()=> { props.Base.focusInputError('form_coupon_in'), listenEvent() });
    }
    else {
...
const callApiForm = async (api: any, stateForm: string = '') => {
  errorApi.value = true;
  await props.Base.apiService.callApi(api, coupon.value ,async (response: any) => { 
    if (props.Base.StateForm === ActionTable.Add || props.Base.StateForm === ActionTable.Replication){
      props.Base.store.dispatch(`${props.Base.Module}/addRecordAction`, { ...coupon.value, couponID: response });
      props.AddItem({ ...coupon.value, couponID: response });
    }
    else{
      props.Base.store.dispatch(`${props.Base.Module}/editRecordAction`, { ...coupon.value, couponID: response });
      props.Base.setStateForm(ActionTable.Add);
    }
...
function validateInput() {
  return [
    /** Validate code */ 
    coupon.value.couponCode.trim() === "" ? t('validate.empty', { field: t('module.order.couponCode') }) : null,
    /** Validate name */
    coupon.value.couponName.trim() === "" ? t('validate.empty', { field: t('module.order.couponName') }) : null,
    !coupon.value.percent && coupon.value.percent != 0 ? t('validate.empty', { field: t('module.order.percent') }) : null,
    !coupon.value.quantity && coupon.value.quantity != 0 ? t('validate.empty', { field: t('module.order.quantity') }) : null,
    !coupon.value.dateStart ? t('validate.empty', { field: t('module.order.dateStart') }) : null,
    !coupon.value.dateEnd ? t('validate.empty', { field: t('module.order.dateEnd') }) : null,
...
const handleCloseModal = () => {
  try {
    if (JSON.stringify(coupon.value) != JSON.stringify(couponComparison.value)) {
      unListenEvent();
